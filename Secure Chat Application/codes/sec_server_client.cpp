...
void runserver(){
    ...
    //serverAddr.sin_addr.s_addr =inet_addr("172.31.0.2"); 
    serverAddr.sin_port = htons(8084);

    //Alice acts like a server when bob tries to initate a connect with it
...
//cout << "Created tcp socket" <<endl;
    // Set server address and port
    memset(&serverAddr, 0, sizeof(serverAddr));
//cout <<"after memset" <<endl;   
 serverAddr.sin_family = AF_INET;
    //serverAddr.sin_addr.s_addr = inet_addr("172.31.0.3"); // Change to server's IP address
    //const char* sv_hostname = "bob1";
    const char* sv_hostname = serverhn;
    struct hostent* server = gethostbyname(sv_hostname);


    serverAddr.sin_addr.s_addr = *((in_addr_t*)server->h_addr_list[0]); 
//cout <<"after assigning the received ip from gethostbyname"<<endl;
 //serverAddr.sin_addr.s_addr =inet_addr(gethostbyname(sv_hostname)); 
    serverAddr.sin_port = htons(8084);
//cout <<"Before attempting to connect to trudy " <<endl;
    // Connect to the server
    if (connect(sockfd, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) < 0) {
        perror("connect() error");
        exit(EXIT_FAILURE);
    }
  // cout << "connected to trudy " <<endl;    
